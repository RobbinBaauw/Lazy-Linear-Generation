...
pub(crate) struct PropagationContextMut<'a> {
    ...
    pub(crate) analysis_log: Option<&'a mut AnalysisLog>,
...
impl<'a> PropagationContextMut<'a> {
    pub(crate) fn new(
        ...
        analysis_log: Option<&'a mut AnalysisLog>,
    ...
    fn build_reason(&self, reason: Reason) -> Reason {
        let new_reason = if let Some(reification_literal) = self.reification_literal {
            match reason {
...
pub(crate) trait ReadDomains: HasAssignments {
    ...
    fn lower_bound_i64<Var: IntegerVariable>(&self, var: &Var) -> i64 {
        var.lower_bound_i64(self.assignments())
    }
    ...
    fn upper_bound_i64<Var: IntegerVariable>(&self, var: &Var) -> i64 {
        var.upper_bound_i64(self.assignments())
    }
...
impl PropagationContextMut<'_> {
    ...
    pub(crate) fn set_upper_bound<Var: IntegerVariable, R: Into<Reason>>(
        ...
        if (bound as i64) < var.upper_bound_i64(self.assignments) {
            let reason = self.build_reason(reason.into());
            let reason_ref = self.reason_store.push(self.propagator_id, reason);
    ...
    pub(crate) fn set_lower_bound<Var: IntegerVariable, R: Into<Reason>>(
        ...
        if (bound as i64) > var.lower_bound_i64(self.assignments) {
            let reason = self.build_reason(reason.into());
            let reason_ref = self.reason_store.push(self.propagator_id, reason);
