...
impl PropagatorStore {
    ...
    pub(crate) fn iter_propagators_and_keys(
        &self,
    ) -> impl Iterator<Item = (PropagatorId, &Box<dyn Propagator>)> + '_ {
        self.propagators.keys().zip(self.propagators.iter())
    }

    pub(crate) fn iter_propagators_and_keys_mut(
        &mut self,
    ) -> impl Iterator<Item = (PropagatorId, &mut Box<dyn Propagator>)> + '_ {
        self.propagators.keys().zip(self.propagators.iter_mut())
    }
}
