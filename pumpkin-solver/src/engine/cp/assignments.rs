...
use crate::engine::constraint_satisfaction_solver::USE_LLG;
...
#[derive(Clone, Debug)]
pub(crate) enum AuxiliaryVariable {
    // Has an iff relation to the variable
    Linked(DomainId, LinearLessOrEqual),

    // There are no direct relations between ids & constraints,
    // but together they define each other
    Unlinked(Vec<DomainId>, Vec<LinearLessOrEqual>),
}
...
pub struct Assignments {
    ...
    pub(crate) linked_auxiliaries: HashMap<LinearLessOrEqual, DomainId>,
    pub(crate) unlinked_auxiliaries: HashMap<PropagatorId, Vec<DomainId>>,
    pub(crate) new_auxiliaries: Vec<AuxiliaryVariable>,
    pub(crate) new_auxiliary_watches: Vec<(DomainId, PropagatorVarId, DomainEvents)>,
...
impl Default for Assignments {
    fn default() -> Self {
        let mut assignments = Self {
            ...
            linked_auxiliaries: Default::default(),
            unlinked_auxiliaries: Default::default(),
            new_auxiliaries: Default::default(),
            new_auxiliary_watches: Default::default(),
...
impl Assignments {
    ...
    pub(crate) fn watch_aux_variable(
        &mut self,
        var: DomainId,
        propagator_var: PropagatorVarId,
        events: DomainEvents,
    ) {
        self.new_auxiliary_watches
            .push((var, propagator_var, events))
    }

    pub(crate) fn unlinked_aux_variables_for_prop(
        &self,
        propagator_id: PropagatorId,
    ) -> Option<&Vec<DomainId>> {
        self.unlinked_auxiliaries.get(&propagator_id)
    }

    pub(crate) fn new_unlinked_aux_variable(&mut self, propagator_id: PropagatorId) -> DomainId {
        pumpkin_assert_simple!(
            USE_LLG.load(Ordering::Relaxed),
            "Auxiliary created in non-LLG mode"
        );
        ...
        trace!("Adding unlinked aux variable {id}");
        match self.unlinked_auxiliaries.get_mut(&propagator_id) {
            None => {
                let _ = self.unlinked_auxiliaries.insert(propagator_id, vec![id]);
            }
            Some(vars) => {
                vars.push(id);
            }
        ...
        let _ = self.domains.push(IntegerDomain::new(0, 1, id, 1, None));
    ...
    pub(crate) fn new_linked_aux_variable(&mut self, linear_leq: LinearLessOrEqual) -> DomainId {
        pumpkin_assert_simple!(
            USE_LLG.load(Ordering::Relaxed),
            "Auxiliary created in non-LLG mode"
        );

        if let Some(existing_var) = self.linked_auxiliaries.get(&linear_leq) {
            return *existing_var;
        }
        ...
        trace!("Adding linked aux variable {id}: {linear_leq}");
        self.new_auxiliaries
            .push(AuxiliaryVariable::Linked(id, linear_leq.clone()));
        let _ = self.linked_auxiliaries.insert(linear_leq.clone(), id);

        let _ = self
            .domains
            .push(IntegerDomain::new(0, 1, id, 1, Some(linear_leq)));
    ...
    pub(crate) fn debug_create_empty_clone(&self) -> Self {
        ...
        Assignments {
            ...
            linked_auxiliaries: self.linked_auxiliaries.clone(),
            unlinked_auxiliaries: self.unlinked_auxiliaries.clone(),
            new_auxiliaries: Default::default(),
            new_auxiliary_watches: Default::default(),
...
impl Assignments {
    pub(crate) fn get_auxiliary_linleq(&self, domain_id: DomainId) -> &Option<LinearLessOrEqual> {
        &self.domains[domain_id].auxiliary_linleq
    }
...
mod tests {
    ...
    fn value_can_be_removed_from_domains() {
        ...
        let mut domain = IntegerDomain::new(1, 5, DomainId::new(0), 0, None);
    ...
    fn get_domain1() -> (DomainId, IntegerDomain, EventSink) {
        ...
        let mut domain = IntegerDomain::new(0, 100, domain_id, 0, None);
    ...
    fn inconsistent_bound_updates() {
        ...
        let domain_id = DomainId::new(0);
        let mut domain = IntegerDomain::new(0, 2, domain_id, 0, None);
    ...
    fn domain_iterator_simple() {
        let domain_id = DomainId::new(0);
        let domain = IntegerDomain::new(0, 5, domain_id, 0, None);
    ...
    fn domain_iterator_skip_holes() {
        ...
        events.grow();
        let domain_id = DomainId::new(0);
        let mut domain = IntegerDomain::new(0, 5, domain_id, 0, None);
    ...
    fn domain_iterator_removed_values_present_beyond_bounds() {
        ...
        events.grow();
        let domain_id = DomainId::new(0);
        let mut domain = IntegerDomain::new(0, 10, domain_id, 0, None);
