...
mod tests {
    ...
    fn propagator_detects_conflict() {
        ...
        assert!(match result {
            Err(Inconsistency::Conflict(x)) => {
                ...
                expected
                    .iter()
                    .all(|y| x.0.iter().collect::<Vec<&Predicate>>().contains(&y))
                    && x.0.iter().all(|y| expected.contains(y))
            }
            _ => false,
        });
    }
        ...
        assert!({
                ...
                &result
            {
                if let Err(Inconsistency::Conflict (explanation_scratch)) =
                    &result_scratch
                {
                    explanation.0.iter().collect::<Vec<_>>()
                        == explanation_scratch.0.iter().collect::<Vec<_>>()
                } else {
                    false
                }
            } else {
    ...
    fn no_synchronisation_leads_to_different_conflict() {
        ...
        ) = (result, result_scratch)
        {
            ...
            let explanation_vec = explanation.0.iter().cloned().collect::<Vec<_>>();
            let explanation_scratch_vec = explanation_scratch.0.iter().cloned().collect::<Vec<_>>();
