...
impl NogoodPropagator {
    ...
    fn log_propagation(
        ...
        if let Some(log) = &mut context.analysis_log {
            if !permanent_nogoods.contains(&nogood_id) {
                log.log_propagation(LearnedItemId::Nogood(nogood_id), context.assignments);
            }
        }
    }
                ...
                IntDomainEvent::LowerBound => {
                    ...
                    while current_index < num_watchers {
                        ...
                        if old_lower_bound < right_hand_side && right_hand_side <= new_lower_bound {
                            ...
                            Self::log_propagation(nogood_id, &self.permanent_nogoods, context);
    ...
    pub(crate) fn add_asserting_nogood(
        ...
        if nogood.len() == 1 {
            ...
            if let Some(analysis_log) = &mut context.analysis_log {
                analysis_log.log_new_learned(LearnedItemId::PermanentNogood);
            }
        ...
        if skip_nogood_learning {
            ...
            if let Some(analysis_log) = &mut context.analysis_log {
                analysis_log.log_new_learned(LearnedItemId::Nogood(new_id));
            }
